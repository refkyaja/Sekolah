...
<div class="min-h-screen bg-background-light dark:bg-background-dark p-8 pb-8">
    <div class="max-w-7xl mx-auto">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="text-slate-800 dark:text-slate-200 text-sm font-bold">Tambah Data Guru</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Data Guru Baru</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Silakan lengkapi formulir di bawah ini untuk menambahkan data tenaga pendidik atau staf.</p>
            </div>
        ...
        <form action="{{ route('admin.guru.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label for="nama" class="text-sm font-bold text-slate-700 dark:text-slate-300">Nama Lengkap *</label>
                        <input type="text" id="nama" name="nama" required value="{{ old('nama') }}"
                               class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm dark:text-slate-200 transition-all"
                               placeholder="Masukkan nama lengkap beserta gelar">
                        @error('nama') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="space-y-2">
                        <label for="nip" class="text-sm font-bold text-slate-700 dark:text-slate-300">NIP</label>
                    ...
                    <div class="space-y-2">
                        <label for="tempat_lahir" class="text-sm font-bold text-slate-700 dark:text-slate-300">Tempat Lahir</label>
                        <input type="text" id="tempat_lahir" name="tempat_lahir" value="{{ old('tempat_lahir') }}"
                               class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm dark:text-slate-200 transition-all"
                               placeholder="Kota Kelahiran">
                    </div>
                    <div class="space-y-2">
                        <label for="tanggal_lahir" class="text-sm font-bold text-slate-700 dark:text-slate-300">Tanggal Lahir *</label>
                        <input type="date" id="tanggal_lahir" name="tanggal_lahir" required value="{{ old('tanggal_lahir') }}"
                               class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm dark:text-slate-200 transition-all" [style.color-scheme]="'dark'">
                        @error('tanggal_lahir') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-bold text-slate-700 dark:text-slate-300 block">Jenis Kelamin *</label>
                        <div class="flex items-center gap-6 h-[42px]">
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <input type="radio" name="jenis_kelamin" value="L" required
                                       ...
                                       class="w-4 h-4 text-primary border-slate-300 dark:border-slate-600 dark:bg-slate-700 focus:ring-primary/20">
                                <span class="text-sm text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Laki-laki</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <input type="radio" name="jenis_kelamin" value="P"
                                       {{ old('jenis_kelamin') == 'P' ? 'checked' : '' }}
                                       class="w-4 h-4 text-primary border-slate-300 dark:border-slate-600 dark:bg-slate-700 focus:ring-primary/20">
                                <span class="text-sm text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Perempuan</span>
                            </label>
                        </div>
                        @error('jenis_kelamin') <p class="text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                    <div class="space-y-2">
                        <label for="agama" class="text-sm font-bold text-slate-700 dark:text-slate-300">Agama</label>
                    ...
                    <div class="space-y-2">
                        <label for="no_hp" class="text-sm font-bold text-slate-700 dark:text-slate-300">No HP *</label>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-700">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <h2 class="text-lg font-bold text-slate-800 dark:text-slate-100">Data Kepegawaian</h2>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label for="jabatan" class="text-sm font-bold text-slate-700 dark:text-slate-300">Jabatan *</label>
                    ...
                    <div id="kelompok-container" class="space-y-2 {{ old('jabatan') == 'guru' ? '' : 'hidden' }}">
                        <label for="kelompok" class="text-sm font-bold text-slate-700 dark:text-slate-300">Kelompok</label>
                    ...
                    <div class="space-y-2">
                        <label for="pendidikan_terakhir" class="text-sm font-bold text-slate-700 dark:text-slate-300">Pendidikan Terakhir</label>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <div class="space-y-2">
                    <label for="alamat" class="text-sm font-bold text-slate-700 dark:text-slate-300">Alamat Lengkap *</label>
                    <textarea id="alamat" name="alamat" rows="4" required
                              class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm dark:text-slate-200 transition-all resize-none"
            ...
            <div class="flex items-center justify-end gap-4 py-4">
                <a href="{{ route('admin.guru.index') }}"
                   class="px-8 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                    Batal
                </a>
                <button type="submit"
