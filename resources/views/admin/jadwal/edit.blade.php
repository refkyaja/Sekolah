...
<div class="max-w-3xl mx-auto">
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5 sm:p-8">
        <div class="mb-8">
            <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">Edit Jadwal</h2>
            <p class="text-sm text-slate-500">Silakan perbarui formulir di bawah untuk mengubah jadwal pelajaran.</p>
        </div>

        <form method="POST" action="{{ route('admin.jadwal-pelajaran.update', $jadwalPelajaran) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="tahun_ajaran_id" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all">
                        @foreach($daftarTahunAjaran as $ta)
                            <option value="{{ $ta->id }}" {{ old('tahun_ajaran_id', $jadwalPelajaran->tahun_ajaran_id) == $ta->id ? 'selected' : '' }}>
                                {{ $ta->tahun_ajaran }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="kelompok" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all">
                        <option value="A" {{ old('kelompok', $jadwalPelajaran->kelompok) == 'A' ? 'selected' : '' }}>Kelompok A</option>
                        <option value="B" {{ old('kelompok', $jadwalPelajaran->kelompok) == 'B' ? 'selected' : '' }}>Kelompok B</option>
                    </select>
                </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="mata_pelajaran" value="{{ old('mata_pelajaran', $jadwalPelajaran->mata_pelajaran) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="time" name="jam_mulai" value="{{ old('jam_mulai', date('H:i', strtotime($jadwalPelajaran->jam_mulai))) }}" 
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="time" name="jam_selesai" value="{{ old('jam_selesai', date('H:i', strtotime($jadwalPelajaran->jam_selesai))) }}" 
                           class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="kategori" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all">
                        <option value="akademik" {{ old('kategori', $jadwalPelajaran->kategori) == 'akademik' ? 'selected' : '' }}>Akademik</option>
                        <option value="art" {{ old('kategori', $jadwalPelajaran->kategori) == 'art' ? 'selected' : '' }}>Art / Seni</option>
                        <option value="physical" {{ old('kategori', $jadwalPelajaran->kategori) == 'physical' ? 'selected' : '' }}>Physical / Olahraga</option>
                        <option value="break" {{ old('kategori', $jadwalPelajaran->kategori) == 'break' ? 'selected' : '' }}>Istirahat</option>
                        <option value="special" {{ old('kategori', $jadwalPelajaran->kategori) == 'special' ? 'selected' : '' }}>Khusus / Acara</option>
                    </select>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="guru" value="{{ old('guru', $jadwalPelajaran->guru) }}" 
                           class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all">
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="lokasi" value="{{ old('lokasi', $jadwalPelajaran->lokasi) }}" 
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all">
            </div>

            {{-- Actions --}}
            <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4">
                <a href="{{ route('admin.jadwal-pelajaran.index', ['kelompok' => $jadwalPelajaran->kelompok]) }}" 
