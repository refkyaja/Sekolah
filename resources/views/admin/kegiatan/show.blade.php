...
<main class="max-w-6xl mx-auto">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            <h2 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100">{{ $kegiatan->nama_kegiatan }}</h2>
            ...
            @else
                <span class="inline-flex items-center px-3 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-full text-[10px] font-bold uppercase tracking-wide">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            <a href="{{ route($routePrefix . '.kegiatan.index') }}" class="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-600 shadow-sm">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden mb-8">
        ...
        <div class="p-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 p-6 bg-slate-50 dark:bg-slate-900 rounded-3xl mb-10 border border-slate-100 dark:border-slate-700">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Lokasi</p>
                        <p class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ $kegiatan->lokasi }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary shrink-0">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Kategori</p>
                        <p class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ $kegiatan->kategori }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div class="lg:col-span-2 space-y-8">
                    <div>
                        ...
                        <div class="text-slate-600 dark:text-slate-400 leading-relaxed space-y-4 whitespace-pre-line">
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            <div class="p-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-2xl italic text-xs text-slate-400 dark:text-slate-500 text-center">
                    ...
                    <div class="p-6 bg-primary/5 rounded-3xl border border-primary/10">
                        ...
                        @if($kegiatan->is_published)
                            <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-4">Kegiatan ini sudah dipublikasikan dan dapat dilihat oleh publik di website sekolah.</p>
                            @if($canUpdateKegiatan)
                            <form action="{{ route($routePrefix . '.kegiatan.toggle-publish', $kegiatan) }}" method="POST">
                                @csrf @method('PATCH')
                                <button type="submit" class="w-full py-2.5 bg-slate-200 text-slate-600 dark:text-slate-400 rounded-xl text-xs font-bold hover:bg-slate-300 transition-all">Sembunyikan (Set Draft)</button>
                            </form>
                            @endif
                        @else
                            <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-4">Kegiatan ini saat ini berstatus Draft dan tidak akan muncul di website publik sampai Anda mengubah statusnya menjadi Public.</p>
                            @if($canUpdateKegiatan)
                            <form action="{{ route($routePrefix . '.kegiatan.toggle-publish', $kegiatan) }}" method="POST">
                                @csrf @method('PATCH')
