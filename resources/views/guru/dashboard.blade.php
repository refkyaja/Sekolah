...
<div class="relative bg-gradient-to-r from-primary to-purple-800 rounded-2xl p-6 sm:p-8 mb-6 sm:mb-8 overflow-hidden min-h-[200px] sm:min-h-[220px] flex items-center shadow-xl shadow-primary/20">
    ...
    <div class="absolute right-0 top-0 bottom-0 w-1/3 flex items-center justify-center opacity-20 pointer-events-none">
        <span class="material-symbols-outlined text-[120px] sm:text-[160px] text-white">school</span>
    </div>
</div>
...
<div class="grid grid-cols-12 gap-6 sm:gap-8">
    ...
    <div class="col-span-12 lg:col-span-8 space-y-6 sm:space-y-8">
        ...
        <div id="stats-section" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 scroll-mt-24">
            <a href="{{ route('guru.siswa.siswa-aktif.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
            ...
            <a href="{{ route('guru.absensi.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 text-primary flex items-center justify-center mb-4">
                    <span class="material-symbols-outlined">how_to_reg</span>
                </div>
                <h3 class="text-slate-400 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Absensi</h3>
            ...
            <a href="{{ route('guru.kalender-akademik.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 text-primary flex items-center justify-center mb-4">
                    <span class="material-symbols-outlined">event_note</span>
                </div>
                <h3 class="text-slate-400 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Kalender</h3>
                <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $stats['total_kalender'] ?? 0 }}</p>
                <p class="text-[10px] text-slate-400 font-bold mt-2">Kegiatan akademik</p>
            </a>

            <a href="{{ route('guru.galeri.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 text-primary flex items-center justify-center mb-4">
                    <span class="material-symbols-outlined">photo_library</span>
                </div>
                <h3 class="text-slate-400 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Galeri</h3>
                <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $stats['total_galeri'] ?? 0 }}</p>
                <p class="text-[10px] text-slate-400 font-bold mt-2">Foto & video</p>
            </a>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6 sm:p-8 border border-slate-100 dark:border-slate-700">
            <div class="flex items-center justify-between mb-6 sm:mb-8">
                <h2 class="text-lg font-bold text-slate-800 dark:text-slate-100">Kegiatan Terbaru</h2>
                <a href="{{ route('guru.kegiatan.index') }}" class="text-primary text-xs font-bold hover:underline">Lihat Semua</a>
            </div>
            <div class="space-y-6 sm:space-y-8">
                @forelse($kegiatan_terbaru ?? [] as $item)
                <div class="flex items-start gap-4">
                    <div class="w-10 h-10 rounded-2xl bg-lavender dark:bg-purple-900/30 flex-shrink-0 flex items-center justify-center">
                    ...
                    @if($item->status)
                    <span class="text-[10px] px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-lg font-bold uppercase flex-shrink-0">{{ $item->status }}</span>
                    @endif
                </div>
                @empty
                <div class="flex items-start gap-4">
                    <div class="w-10 h-10 rounded-2xl bg-slate-100 dark:bg-slate-700 flex-shrink-0 flex items-center justify-center">
                        <span class="material-symbols-outlined text-slate-400 text-xl">event_busy</span>
                    </div>
                    <div class="flex-1">
                        <p class="text-sm text-slate-500 dark:text-slate-400">Belum ada kegiatan terbaru.</p>
                    </div>
                </div>
                @endforelse
            </div>
    ...
    <div class="col-span-12 lg:col-span-4 space-y-6 sm:space-y-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-5 sm:p-6 border border-slate-100 dark:border-slate-700 flex flex-col items-center text-center">
            ...
            <div class="w-full flex justify-between px-4 mb-5 sm:mb-6">
                <div>
                    <p class="text-lg font-bold text-primary">{{ $stats['siswa_aktif'] ?? 0 }}</p>
                    <p class="text-[10px] text-slate-400 uppercase font-black">Siswa</p>
                </div>
                <div class="h-8 w-px bg-slate-100 dark:bg-slate-700"></div>
                <div>
                    <p class="text-lg font-bold text-primary">{{ $stats['persen_absen'] ?? 0 }}%</p>
                    <p class="text-[10px] text-slate-400 uppercase font-black">Absensi</p>
                </div>
                <div class="h-8 w-px bg-slate-100 dark:bg-slate-700"></div>
                <div>
                    <p class="text-lg font-bold text-primary">{{ $stats['total_kegiatan'] ?? 0 }}</p>
                    <p class="text-[10px] text-slate-400 uppercase font-black">Kegiatan</p>
                </div>
            </div>
            <a href="{{ route('guru.profile.index') }}" class="w-full py-3 bg-lavender dark:bg-primary/20 text-primary font-bold text-sm rounded-xl hover:bg-primary hover:text-white transition-all">
                Lihat Profil
            </a>
        </div>
    </div>
