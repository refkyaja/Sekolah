...
<div class="max-w-4xl mx-auto">
            ...
            <div class="flex-shrink-0">
                <div class="relative">
                    ...
                    <img src="{{ Storage::url($user->foto) }}" 
                         alt="{{ $user->name }}" 
                         class="w-24 h-24 rounded-2xl object-cover shadow-sm ring-4 ring-green-100 dark:ring-green-900/30">
                    @else
                    <div class="w-24 h-24 rounded-2xl bg-green-600 flex items-center justify-center text-3xl font-bold text-white shadow-sm ring-4 ring-green-100 dark:ring-green-900/30">
                        {{ strtoupper(substr($user->name, 0, 1)) }}
                    </div>
                    @endif
                    
                    <!-- Upload Photo Button -->
                    <form action="{{ route('kepala-sekolah.profile.update-photo') }}" method="POST" enctype="multipart/form-data" class="absolute -bottom-2 -right-2">
                        @csrf
                        <label for="foto" class="block w-10 h-10 bg-green-600 text-white rounded-full flex items-center justify-center cursor-pointer hover:bg-green-700 transition-colors shadow-sm">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6 border border-slate-100 dark:border-slate-700 mb-6">
        ...
        <form action="{{ route('kepala-sekolah.profile.update') }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           class="w-full px-4 py-2 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-slate-700 dark:text-white transition-colors"
                           required>
                    @error('name')
            ...
            <div class="mt-6">
                ...
                <textarea id="alamat" 
                          name="alamat" 
                          rows="3" 
                          class="w-full px-4 py-2 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-slate-700 dark:text-white transition-colors">{{ old('alamat', $user->alamat) }}</textarea>
                @error('alamat')
                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6 border border-slate-100 dark:border-slate-700">
        ...
        <form action="{{ route('kepala-sekolah.profile.change-password') }}" method="POST">
            @csrf
            @method('PUT')
