<!DOCTYPE html>
<html lang="id" x-data="{ sidebarCollapsed: localStorage.getItem('guruSidebarCollapsed') === 'true' }" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>Guru Panel - {{ config('app.name', 'TK PGRI Harapan Bangsa 1') }}</title>
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    ...
    <script>
        (function() {
            try {
                var savedTheme = localStorage.getItem('guruTheme') || 'system';
                ...
                window.applyGuruTheme = function() {
                    var currentTheme = localStorage.getItem('guruTheme') || 'system';
                ...
                window.addEventListener('storage', function(event) {
                    if (!event.key || event.key === 'guruTheme') {
                        window.applyGuruTheme();
                    }
                });

                window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', function() {
                    if ((localStorage.getItem('guruTheme') || 'system') === 'system') {
                        window.applyGuruTheme();
                    }
                });
            } catch (e) {}
            ...
            <main class="flex-1 overflow-y-auto px-8 pb-8">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap items-center gap-x-2 gap-y-1 mt-2 text-sm text-slate-500 dark:text-slate-400">
                        <a href="{{ route('guru.dashboard') }}" class="hover:text-primary transition-colors shrink-0">Dashboard</a>
