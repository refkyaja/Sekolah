<!DOCTYPE html>
<html lang="id" x-data="{ sidebarCollapsed: localStorage.getItem('operatorSidebarCollapsed') === 'true' }" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>Operator Panel - {{ config('app.name', 'TK PGRI Harapan Bangsa 1') }}</title>
...
<body class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen">
    ...
    <div class="flex h-screen overflow-hidden">
        @include('layouts.partials.operator-sidebar')

        <div class="flex-1 flex flex-col min-w-0 overflow-y-hidden">
            @include('layouts.partials.operator-header')
            ...
            <main class="flex-1 overflow-y-auto px-8 pb-8">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap items-center gap-x-2 gap-y-1 mt-2 text-sm text-slate-500 dark:text-slate-400">
                        <a href="{{ route('operator.dashboard') }}" class="hover:text-primary transition-colors shrink-0">Dashboard</a>
