<div class="relative" x-data="{ open: $wire.entangle('isOpen') }" @click.outside="open = false; $wire.closeDropdown()">
    ...
    <button
        ...
        class="p-2.5 text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800/80 rounded-2xl transition-all shadow-sm relative"
    ...
    <div
        ...
        x-cloak
        class="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 z-[60] overflow-hidden"
        style="display: none;"
    >
        {{-- Header Dropdown --}}
        ...
        <div class="max-h-72 overflow-y-auto divide-y divide-slate-50 dark:divide-slate-800">
            ...
            <div
                wire:key="{{ $notif['id'] }}"
                class="flex gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {{ $notif['is_unread'] ? 'bg-primary/5 dark:bg-primary/10' : '' }} cursor-pointer group"
                wire:click="markRead('{{ $notif['id'] }}')"
            >
                {{-- Icon berdasarkan tipe --}}
                <div class="flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center
                    @if(str_contains($notif['type'], 'ppdb') || str_contains($notif['type'], 'spmb')) bg-blue-100 dark:bg-blue-900/30 text-blue-600
                    @elseif(str_contains($notif['type'], 'lulus')) bg-green-100 dark:bg-green-900/30 text-green-600
                    @elseif(str_contains($notif['type'], 'revisi') || str_contains($notif['type'], 'tolak')) bg-red-100 dark:bg-red-900/30 text-red-600
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-1">
                        <p class="text-xs font-semibold text-slate-800 dark:text-slate-100 truncate">{{ $notif['title'] }}</p>
                        @if($notif['is_unread'])
                        <span class="flex-shrink-0 w-2 h-2 bg-primary rounded-full mt-1"></span>
                        @endif
                    </div>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{{ $notif['body'] }}</p>
            ...
            <div class="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p class="text-sm text-slate-400 dark:text-slate-500 font-medium">Belum ada notifikasi</p>
            </div>
            @endforelse
        </div>
        ...
        <div class="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            <p class="text-[11px] text-slate-400 text-center">Menampilkan {{ count($notifications) }} notifikasi terbaru</p>
        </div>
        @endif
    </div>
