<div class="notification-system-card flex h-full max-h-[42rem] min-h-[32rem] flex-col overflow-hidden rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <style>
    ...
    <div class="notification-scroll flex-1 overflow-y-auto pl-3 pr-2">
        <div class="space-y-5">
            ...
            @php
                ...
                $timelineDot = $loop->first
                    ...
                    : 'bg-white dark:bg-slate-800 border-2 border-primary/35 dark:bg-slate-900';
            ...
            >
                ...
                <div class="mb-1 flex items-center justify-between gap-2">
                    ...
                    <span class="text-[10px] text-slate-400 dark:text-slate-500 whitespace-nowrap">{{ $notif['time_ago'] }}</span>
                </div>
                <p class="text-sm font-semibold mb-1 {{ $notif['is_unread'] ? 'text-slate-800 dark:text-slate-100 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400' }}">
                    {{ $notif['title'] }}
                </p>
                <p class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 leading-relaxed">{{ $notif['body'] }}</p>

                @if(isset($notif['data']['note']))
                <div class="mt-3 rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-900/10 px-3 py-2 text-xs text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
                    {{ $notif['data']['note'] }}
                </div>
                @endif
            ...
            <div class="flex flex-col items-center justify-center py-8 text-center">
                <span class="material-symbols-outlined text-5xl text-slate-200 dark:text-slate-700 dark:text-slate-700 mb-3">notifications_off</span>
                <p class="text-sm font-semibold text-slate-800 dark:text-slate-100 dark:text-slate-100 mb-1">Belum ada notifikasi sistem</p>
        ...
        <details class="mt-6 rounded-2xl border border-slate-200 dark:border-slate-600 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-800/40">
            <summary class="cursor-pointer list-none text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-200">
                Lainnya
                <span class="ml-2 text-xs font-medium text-slate-400 dark:text-slate-500">({{ count($olderNotifications) }} notifikasi lama)</span>
            </summary>
            <div class="notification-scroll mt-4 max-h-72 space-y-5 overflow-y-auto pl-3 pr-2">
                ...
                @php
                    ...
                    $timelineDot = 'bg-white dark:bg-slate-800 border-2 border-primary/25 dark:bg-slate-900';
                    $labelColor = 'text-primary';
                @endphp
                <div
