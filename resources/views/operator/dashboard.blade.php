...
<div class="relative bg-gradient-to-r from-primary to-purple-800 rounded-2xl p-6 sm:p-8 mb-6 sm:mb-8 overflow-hidden min-h-[200px] sm:min-h-[220px] flex items-center shadow-xl shadow-primary/20">
    <div class="relative z-10 max-w-lg">
        ...
        <p class="text-white/80 mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base">
            Selamat datang di dashboard Operator. Akses ringkasan data, kelola informasi, dan pantau operasional sekolah.
        </p>
...
<div class="grid grid-cols-12 gap-6 sm:gap-8">
    ...
    <div class="col-span-12 lg:col-span-8 space-y-6 sm:space-y-8">
        ...
        <div id="stats-section" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 scroll-mt-24">
            <a href="{{ route('operator.siswa.siswa-aktif.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
            ...
            <a href="{{ route('operator.tahun-ajaran.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                ...
                <p class="text-[10px] text-blue-500 font-bold mt-2 flex items-center gap-1">
                    ...
                    Aktif: {{ $tahun_ajaran_aktif ? $tahun_ajaran_aktif->tahun_ajaran : 'Tidak Ada' }}
                </p>
            </a>
            ...
            <a href="{{ route('operator.ppdb.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 text-primary flex items-center justify-center mb-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <a href="{{ route('operator.jadwal-pelajaran.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                ...
                <h3 class="text-slate-400 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Jadwal</h3>
            ...
            <a href="{{ route('operator.galeri.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 text-primary flex items-center justify-center mb-4">
            ...
            <a href="{{ route('operator.bukutamu.index') }}" class="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all border border-slate-100 dark:border-slate-700">
                <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 text-primary flex items-center justify-center mb-4">
                    <span class="material-symbols-outlined">book</span>
                </div>
                <h3 class="text-slate-400 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Buku Tamu</h3>
                <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $stats['total_bukutamu'] ?? 0 }}</p>
                <p class="text-[10px] text-slate-400 font-bold mt-2">Pengunjung</p>
            </a>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6 sm:p-8 border border-slate-100 dark:border-slate-700">
            <div class="flex items-center justify-between mb-6 sm:mb-8">
                ...
                <a href="{{ route('operator.ppdb.index') }}" class="text-primary text-xs font-bold hover:underline">Lihat Semua</a>
            </div>
    ...
    <div class="col-span-12 lg:col-span-4 space-y-6 sm:space-y-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-5 sm:p-6 border border-slate-100 dark:border-slate-700 flex flex-col items-center text-center">
            ...
            <div class="w-full flex justify-between px-4 mb-5 sm:mb-6">
                ...
                <div class="h-8 w-px bg-slate-100 dark:bg-slate-700"></div>
                <div>
                    <p class="text-lg font-bold text-primary">{{ $stats['total_materi'] ?? 0 }}</p>
                    <p class="text-[10px] text-slate-400 uppercase font-black">Materi</p>
                </div>
            </div>
            <a href="{{ route('operator.profile.index') }}" class="w-full py-3 bg-lavender dark:bg-primary/20 text-primary font-bold text-sm rounded-xl hover:bg-primary hover:text-white transition-all">
                Lihat Profil
            </a>
        </div>
    </div>
