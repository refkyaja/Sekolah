...
@section('title', 'Verifikasi Kode OTP - Portal Siswa')
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-display">
    <div class="sm:mx-auto sm:w-full sm:max-w-md text-center px-4">
        ...
        <h2 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Verifikasi Kode</h2>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
        <div class="bg-white dark:bg-slate-900 py-10 px-8 shadow-2xl shadow-slate-200/50 dark:shadow-none sm:rounded-[3rem] border border-slate-100 dark:border-slate-800">
            ...
            <form class="space-y-6" action="{{ route('siswa.password.verify.submit') }}" method="POST">
                @csrf
                <div class="space-y-2 text-center">
                    <label for="otp" class="text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Kode OTP</label>
                    <div class="relative group">
                        <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors">verified</span>
                        <input id="otp" name="otp" type="text" maxlength="6" pattern="\d{6}" required autofocus
                            class="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all dark:text-white text-center text-4xl font-bold tracking-[0.5em]" 
                            placeholder="000000">
                    </div>
                    @error('otp')
            ...
            <div class="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800 text-center">
                <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">Tidak menerima kode?</p>
                <form action="{{ route('siswa.password.otp') }}" method="POST">
                    @csrf
                    <input type="hidden" name="email" value="{{ session('reset_email') }}">
                    <button type="submit" class="font-bold text-primary hover:underline transition-all text-sm">Kirim Ulang Kode OTP</button>
                </form>
            </div>
        </div>
    </div>
